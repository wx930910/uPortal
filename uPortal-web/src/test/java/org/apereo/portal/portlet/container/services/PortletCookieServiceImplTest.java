...
public class PortletCookieServiceImplTest {
	...
	/**
	 * Control test invocation of
	 * {@link PortletCookieServiceImpl#updatePortalCookie(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)}.
	 */
	@Test
	public void testUpdatePortletCookieControl() {
		...
		String[] portalCookieValue = new String[1];
		DateTime[] portalCookieExpires = new DateTime[] { DateTime.now().plusHours(1) };
		DateTime[] portalCookieCreated = new DateTime[] { DateTime.now() };
		Set<IPortletCookie>[] portalCookiePortletCookies = new Set[] { new HashSet<IPortletCookie>() };
		when(portalCookie.getCreated()).thenAnswer((stubInvo) -> {
			return portalCookieCreated[0];
		});
		when(portalCookie.getExpires()).thenAnswer((stubInvo) -> {
			return portalCookieExpires[0];
		});
		doAnswer((stubInvo) -> {
			DateTime expires = stubInvo.getArgument(0);
			portalCookieExpires[0] = expires;
			return null;
		}).when(portalCookie).setExpires(any());
		when(portalCookie.getValue()).thenAnswer((stubInvo) -> {
			return portalCookieValue[0];
		});
		when(portalCookie.getPortletCookies()).thenAnswer((stubInvo) -> {
			return portalCookiePortletCookies[0];
		});
		portalCookieValue[0] = "ABCDEF";
		EasyMock.expect(portletCookieDao.createPortalCookie(PortletCookieServiceImpl.DEFAULT_MAX_AGE))
				.andReturn(portalCookie);
		EasyMock.expect(
				portletCookieDao.updatePortalCookieExpiration(portalCookie, PortletCookieServiceImpl.DEFAULT_MAX_AGE))
				.andReturn(portalCookie);
	...
	/**
	 * Test
	 * {@link PortletCookieServiceImpl#getOrCreatePortalCookie(javax.servlet.http.HttpServletRequest)}.
	 ...
	 * Test
	 * {@link PortletCookieServiceImpl#getOrCreatePortalCookie(javax.servlet.http.HttpServletRequest)}.
	 * that results in returning an existing portalcookie from the id stored in the
	 * session.
	 */
	@Test
	...
	/**
	 * Control test for adding a portlet cookie: no existing portalCookie, portlet
	 * cookie requires persistence.
	 */
	@Test
	...
	/**
	 * Control test for removing a portlet cookie. Logic is nearly identical to
	 * create, as both
	 * {@link IPortletCookieDao#addOrUpdatePortletCookie(IPortalCookie, Cookie)} is
	...
	/**
	 * Control test for adding a portlet cookie with maxAge == -1, which results in
	...
	public void testAddCookieSessionOnly() {
		...
		Map<String, SessionOnlyPortletCookieImpl> sessionOnlyMap = cookieService
