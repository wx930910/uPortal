...
public class AuthorizationTester extends TestCase {
	public IPermissionPolicy mockIPermissionPolicy2() {
		IPermissionPolicy mockInstance = mock(IPermissionPolicy.class);
		try {
			when(mockInstance.toString()).thenReturn(mockInstance.getClass().getName());
			when(mockInstance.doesPrincipalHavePermission(any(IAuthorizationService.class),
					any(IAuthorizationPrincipal.class), any(IPermissionOwner.class), any(IPermissionActivity.class),
					any(IPermissionTarget.class))).thenAnswer((stubInvo) -> {
						IAuthorizationService service = stubInvo.getArgument(0);
						IAuthorizationPrincipal principal = stubInvo.getArgument(1);
						IPermissionOwner owner = stubInvo.getArgument(2);
						IPermissionActivity activity = stubInvo.getArgument(3);
	...
	public IPermissionPolicy mockIPermissionPolicy1() {
		IPermissionPolicy mockInstance = mock(IPermissionPolicy.class);
		try {
			when(mockInstance.toString()).thenReturn(mockInstance.getClass().getName());
			when(mockInstance.doesPrincipalHavePermission(any(IAuthorizationService.class),
	...
	private IPermissionPolicy getNegativePermissionPolicy() throws AuthorizationException {
		if (negativePermissionPolicy == null) {
			negativePermissionPolicy = mockIPermissionPolicy1();
		}
	...
	private IPermissionPolicy getPositivePermissionPolicy() throws AuthorizationException {
		if (positivePermissionPolicy == null) {
			positivePermissionPolicy = mockIPermissionPolicy2();
		}
	...
	private void initializePermissionStore() throws AuthorizationException {
		...
		String factoryName = PropertiesManager
				.getProperty("org.apereo.portal.security.IPermissionStore.implementation");
		if (factoryName == null) {
		...
		try {
			...
			eMsg = "AuthorizationTester.initializePermissionStore(): Problem creating permission store... "
					+ e.getMessage();
			print(eMsg);
			throw new AuthorizationException(eMsg);
		}
	...
	protected void setUp() {
		...
		try {
			...
			retrievedPermissions = getPermissionStore().select(OWNER, EVERYONE_GROUP_PRINCIPAL_KEY, TEST_ACTIVITY, null,
			...
			retrievedPermissions = getPermissionStore().select(OWNER, STUDENT_GROUP_PRINCIPAL_KEY, TEST_ACTIVITY,
					retrievedPermission.getTarget(), IPermission.PERMISSION_TYPE_DENY);
			assertEquals(msg, 0, retrievedPermissions.length);
			getPermissionStore().add(newPermission);
			retrievedPermissions = getPermissionStore().select(OWNER, STUDENT_GROUP_PRINCIPAL_KEY, TEST_ACTIVITY,
					retrievedPermission.getTarget(), IPermission.PERMISSION_TYPE_DENY);
			...
			retrievedPermissions = getPermissionStore().select(OWNER, STUDENT_PRINCIPAL_KEY, TEST_ACTIVITY,
					retrievedPermission.getTarget(), IPermission.PERMISSION_TYPE_DENY);
			assertEquals(msg, 0, retrievedPermissions.length);
			getPermissionStore().add(newPermission);
			retrievedPermissions = getPermissionStore().select(OWNER, STUDENT_PRINCIPAL_KEY, TEST_ACTIVITY,
	...
	protected void tearDown() {
		try {
			IPermission[] permissions = (IPermission[]) testPermissions
					.toArray(new IPermission[testPermissions.size()]);
	...
	public void testPermissionStore() throws Exception {
		...
		print(msg);
		permissions = getPermissionStore().select(OWNER, EVERYONE_GROUP_PRINCIPAL_KEY, activity, nonExistingTarget,
				IPermission.PERMISSION_TYPE_GRANT);
