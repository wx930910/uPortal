...
public class ParenTest extends TestCase {
	...
	public static Evaluator mockEvaluator1() {
		Evaluator mockInstance = spy(Evaluator.class);
		doThrow(new UnsupportedOperationException()).when(mockInstance).toElement(any(Element.class));
		doAnswer((stubInvo) -> {
			throw new UnsupportedOperationException();
		}).when(mockInstance).getFactoryClass();
		doReturn("(NO ONE)").when(mockInstance).getSummary();
		return mockInstance;
	}
	...
	public void testIsApplicable() {
		...
		orParen.addEvaluator(ParenTest.mockEvaluator1());
		...
		andParen.addEvaluator(ParenTest.mockEvaluator1());
		...
		notParen.addEvaluator(ParenTest.mockEvaluator1());
